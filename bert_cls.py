...
flags.DEFINE_float("layerwise_lr_decay_power", 0.9,
                   "layerwise learning rate decay power")

flags.DEFINE_float("epsilon", 0.1, "attack range")

flags.DEFINE_float("num_train_epochs", 3.0,
                   "Total number of training epochs to perform.")

flags.DEFINE_float("num_per_epochs", 1000,
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings, reuse=False):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            with tf.control_dependencies(embed_norm_grads):
                ...
                for param, norm_grad in zip(embed_vars, embed_norm_grads):
                    attack_op.append(param.assign(param + tf.multiply(FLAGS.epsilon, norm_grad)))
                attack_op = tf.group(*attack_op, name='attack')

            #  attack后再计算loss, grads
            with tf.control_dependencies([attack_op]):
                (att_total_loss, att_per_example_loss, att_logits, att_probabilities) = create_model(
                    bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,
                    num_labels, use_one_hot_embeddings, reuse=True)
                att_grads = tf.gradients(att_total_loss, tvars, name='att_gradients')  #  返回None

            #  计算attack后的gradient后，进行参数恢复
            with tf.control_dependencies(att_grads):
                restore_op = []
                for param, norm_grad in zip(embed_vars, embed_norm_grads):
                    restore_op.append(param.assign(param - tf.multiply(FLAGS.epsilon, norm_grad)))
                restore_op = tf.group(*restore_op, name='restore')

            #  进行梯度的累计，看做是一个batch的样本求平均
            with tf.control_dependencies([restore_op]):
                acc_grads = [tf.div(tf.add(grad, att_grad), 2.0) for grad, att_grad in zip(grads, att_grads)]
                acc_total_loss = tf.div(tf.add(total_loss, att_total_loss), 2.0)

            #  进行参数的更新，必须在参数恢复后
            with tf.control_dependencies(acc_grads):
                train_op = optimization.create_optimizer(
                    tvars, acc_grads, learning_rate, num_train_steps, num_warmup_steps, use_tpu)

            logging_hook1 = tf.train.LoggingTensorHook({"total_loss": acc_total_loss}, every_n_iter=10)
